import React from 'react'
import classNames from 'classnames'
import { useDispatch } from 'react-redux'
import { setFilterObj } from '../../actions/filters'

const SortPopup = React.memo(() => {
  const [isActivePopup, setIsActivePopup] = React.useState(false)
  const [currentFilter, setCurrentFilter] = React.useState(1)
  const sortPopupRef = React.useRef()

  const dispatch = useDispatch()

  const sortFilters = [
    { id: 1, name: 'популярности', type: 'rating', order: 'desc' },
    { id: 2, name: 'цене', type: 'price', order: 'desc' },
    { id: 3, name: 'алфавиту', type: 'name', order: 'asc' },
  ]

  React.useEffect(() => {
    document.documentElement.addEventListener('click', (e) => {
      if (!e.path.includes(sortPopupRef.current)) {
        setIsActivePopup(false)
      }
    })
  }, [])

  const togglePopup = () => {
    setIsActivePopup((prevVal) => !prevVal)
  }

  const chooseFilter = (filter) => {
    setCurrentFilter(filter.id)
    setIsActivePopup(false)
    dispatch(setFilterObj({ type: filter.type, order: filter.order }))
  }

  return (
    <div className="sort" ref={sortPopupRef}>
      <div className="sort__label">
        <svg
          className={classNames({ active: isActivePopup })}
          width="10"
          height="6"
          viewBox="0 0 10 6"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z"
            fill="#2C2C2C"
          />
        </svg>
        <b>Сортировка по:</b>
        <span onClick={togglePopup}>
          {sortFilters.find((filter) => filter.id === currentFilter).name}
        </span>
      </div>
      {isActivePopup && (
        <div className="sort__popup">
          <ul>
            {sortFilters &&
              sortFilters.map((filter) => (
                <li
                  className={classNames({
                    active: filter.id === currentFilter,
                  })}
                  onClick={() => chooseFilter(filter)}
                  key={filter.id}
                >
                  {filter.name}
                </li>
              ))}
          </ul>
        </div>
      )}
    </div>
  )
})

export default SortPopup
